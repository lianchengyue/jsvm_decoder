...
ErrVal H264AVCDecoderTest::BufferParameters::init(const JSVM::AccessUnit& rcAccessUnit)
{
    const JSVM::SliceDataNALUnit* pcSliceDataNalUnit = rcAccessUnit.getLastVCLNalUnit();
                                 ...
                                 #endif
{
    ...
    JSVM::CreaterH264AVCDecoder* pcH264AVCDecoder = 0;
    JSVM::CreaterH264AVCDecoder::create(pcH264AVCDecoder);
    ...
    m_pcH264AVCDecoder->init(true);

    return Err::m_nOK;
}

ErrVal H264AVCDecoderTest::uninit()
{
    ROF(m_bInitialized);
    ...
    {
        m_pcH264AVCDecoder->uninit(true);
...
ErrVal H264AVCDecoderTest::go()
{
    ...
    JSVM::AccessUnit  cAccessUnit;
    while(! cAccessUnit.isEndOfStream())
    {
        xProcessAccessUnit(cAccessUnit, bFirstAccessUnit, uiNumProcessed);
    }
#ifdef SHARP_AVC_REWRITE_OUTPUT
...
ErrVal H264AVCDecoderTest::xProcessAccessUnit(JSVM::AccessUnit& rcAccessUnit, Bool& rbFirstAccessUnit, UInt& ruiNumProcessed)
{
    ...
    while(!rcAccessUnit.isComplete())
    {
        ...
        m_pcReadBitstream ->extractPacket (pcBinData, bEndOfStream);
        m_pcH264AVCDecoder->initNALUnit   (pcBinData, rcAccessUnit);
    ...
    {
        m_cBufferParameters.init(rcAccessUnit);
    ...
    while(rcAccessUnit.isComplete())
    {
        JSVM::NALUnit*  pcNalUnit   = 0;
        PicBuffer*      pcPicBuffer = 0;
        PicBufferList   cPicBufferOutputList;
        PicBufferList   cPicBufferUnusedList;
        BinDataList     cBinDataList;
        rcAccessUnit.getAndRemoveNextNalUnit(pcNalUnit);
        xGetNewPicBuffer(pcPicBuffer, m_cBufferParameters.getBufferSize());
        m_pcH264AVCDecoder->processNALUnit(pcPicBuffer, cPicBufferOutputList, cPicBufferUnusedList, cBinDataList, *pcNalUnit);
        xOutputNALUnits (cBinDataList, ruiNumProcessed);
        xOutputPicBuffer(cPicBufferOutputList, ruiNumProcessed);
        xRemovePicBuffer(cPicBufferUnusedList);
        delete pcNalUnit;
    }
    return Err::m_nOK;
}
...
ErrVal H264AVCDecoderTest::xOutputNALUnits(BinDataList& rcBinDataList, UInt& ruiNumNALUnits)
{
#ifdef SHARP_AVC_REWRITE_OUTPUT
    while(! rcBinDataList.empty())
    {
        ...
        ROF(pcBinData);
        m_pcWriteBitstream->writePacket(&m_cBinDataStartCode);
        m_pcWriteBitstream->writePacket(pcBinData);
        ruiNumNALUnits++;
...
ErrVal H264AVCDecoderTest::xOutputPicBuffer(PicBufferList& rcPicBufferOutputList, UInt& ruiNumFrames)
{
#ifdef SHARP_AVC_REWRITE_OUTPUT
    ...
    while(! rcPicBufferOutputList.empty())
    {
        ...
        {
            ROF (m_pcLastFrame);
            m_pcWriteYuv->writeFrame(m_pcLastFrame + m_cBufferParameters.getLumaOffset(),
