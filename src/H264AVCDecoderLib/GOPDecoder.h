#ifndef _GOPDECODER_H_
#define _GOPDECODER_H_
...
class CurrDPBUnit : public DPBUnit
{
protected:
...
public:
    ...
    ErrVal reinit(SliceHeader& rcSliceHeader, Bool bNewLayerRepresentation);
    ErrVal resetMMCO5(SliceHeader& rcSliceHeader);
    ErrVal setComplete(CurrDPBUnit& rcILPredUnit, Bool bDependencyRepresentationFinished);
    ErrVal store(DPBUnit& rcDPBUnit, Bool bRefBasePic = false);
    ErrVal uninit();
...
protected:
    ErrVal xCreateData(const SequenceParameterSet& rcSPS, Bool bCreateRefBasePicBuffer, Bool bBaseLayer);
    ErrVal xDeleteData();
    ErrVal xStoreFrame(DPBUnit& rcDPBUnit, Bool bRefBasePic);
    ErrVal xStore2ndField(DPBUnit& rcDPBUnit, Bool bRefBasePic);
...
public:
    ...
    UInt getFrameWidth()  const {
        return m_uiFrameWidthInMb*16;
    }
    UInt getFrameHeight()  const {
        return m_uiFrameHeightInMb*16;
    }
