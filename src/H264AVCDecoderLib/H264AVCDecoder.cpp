...
ErrVal H264AVCDecoder::init(NalUnitParser*      pcNalUnitParser,
...
ErrVal H264AVCDecoder::initNALUnit(BinData*&    rpcBinData,
                                   AccessUnit&  rcAccessUnit)
{
    ...
    {
        rcAccessUnit.update(0, false, false, false);
        return Err::m_nOK;
    }
    ...
    Bool          bBufferingPeriod  = false;
    {
        ...
        switch(m_pcNalUnitParser ->getNalUnitType())
        {
            ...
            case NAL_UNIT_SUBSET_SPS:
            {
                ...
                pcSPS->read(m_pcHeaderSymbolReadIf, m_pcNalUnitParser->getNalUnitType(), bCompletelyParsed);
                m_pcParameterSetMngAUInit->store(pcSPS);
                break;
            }
            case NAL_UNIT_PPS:
            {
                ...
                pcPPS->read(m_pcHeaderSymbolReadIf, m_pcNalUnitParser->getNalUnitType());
                m_pcParameterSetMngAUInit->store(pcPPS);
            ...
            case NAL_UNIT_CODED_SLICE_IDR:
            {
                ...
                pcSliceHeader->read(*m_pcParameterSetMngAUInit, *m_pcHeaderSymbolReadIf);
                break;
            }
            ...
            case NAL_UNIT_SEI:
            {
                ...
                SEI::read(m_pcHeaderSymbolReadIf, cSEIMessageList, m_pcParameterSetMngAUInit);
                bScalableSEI      = ((*cSEIMessageList.begin())->getMessageType() == JSVM::SEI::SCALABLE_SEI);
                bBufferingPeriod  = ((*cSEIMessageList.begin())->getMessageType() == JSVM::SEI::BUFFERING_PERIOD);
                if((*cSEIMessageList.begin())->getMessageType() == JSVM::SEI::SCALABLE_NESTING_SEI)
                {
        ...
        m_pcNalUnitParser->closeNalUnit(bCompletelyParsed);
        DTRACE_ON;
      }
      ...
      {
          case NAL_UNIT_PREFIX:
              rcAccessUnit.update(pcBinDataCopy, *pcPrefixHeader);
              break;

          case NAL_UNIT_CODED_SLICE:
          case NAL_UNIT_CODED_SLICE_IDR:
          case NAL_UNIT_CODED_SLICE_SCALABLE:
              rcAccessUnit.update(pcBinDataCopy, *pcSliceHeader);
              break;
          ...
          default:
              rcAccessUnit.update(pcBinDataCopy, bSEI, bScalableSEI, bBufferingPeriod);
              break;
    }
    return Err::m_nOK;
}
...
ErrVal H264AVCDecoder::processNALUnit(PicBuffer *pcPicBuffer,
                                ...
                                NALUnit &rcNALUnit)
{
    if(! rcNALUnit.isVCLNALUnit())
    {
        ...
        xProcessNonVCLNALUnit(rcNonVCLNALUnit);
    ...
    //processSliceData()
    m_apcLayerDecoder[ rcSliceDataNALUnit.getDependencyId() ]->processSliceData(pcPicBuffer, rcOutputList, rcPicBufferUnusedList, rcBinDataList, rcSliceDataNALUnit);
    ...
    {
        m_apcLayerDecoder[uiDependencyId]->finishProcess(cDummyList, rcPicBufferUnusedList);
    }
    m_apcLayerDecoder[rcSliceDataNALUnit.getDependencyId()]->finishProcess(rcPicBufferOutputList, rcPicBufferUnusedList);
...
ErrVal H264AVCDecoder::updateDPB(UInt           uiTargetDependencyId,
                           PicBufferList& rcPicBufferOutputList,
                           PicBufferList& rcPicBufferUnusedList)
{
    PicBufferList cDummyList;
    ...
    {
        m_apcLayerDecoder[uiDependencyId]->updateDPB(cDummyList, rcPicBufferUnusedList);
    }

    m_apcLayerDecoder[uiTargetDependencyId]->updateDPB(rcPicBufferOutputList, rcPicBufferUnusedList);
    return  Err::m_nOK;
}

ErrVal H264AVCDecoder::xProcessNonVCLNALUnit(NonVCLNALUnit& rcNonVCLNALUnit)
{
    //===== parse NAL unit =====
    Bool            bCompletelyParsed  = true;
    BinDataAccessor cBinDataAccessor;
    rcNonVCLNALUnit.getBinData()->setMemAccessor(cBinDataAccessor);
    m_pcNalUnitParser->initNalUnit(cBinDataAccessor, (rcNonVCLNALUnit.isScalableSEI     () ? -3 :
    ...
    switch(m_pcNalUnitParser->getNalUnitType())
    {
        ...
        case NAL_UNIT_SPS:
          {
            ...
            SequenceParameterSet::create(pcSPS);
            pcSPS->read(m_pcHeaderSymbolReadIf, m_pcNalUnitParser->getNalUnitType(), bCompletelyParsed);
            m_pcParameterSetMngDecode->store(pcSPS);
        ...
        case NAL_UNIT_PPS:
        {
            ...
            PictureParameterSet::create(pcPPS);
            pcPPS->read(m_pcHeaderSymbolReadIf, m_pcNalUnitParser->getNalUnitType());
            m_pcParameterSetMngDecode->store(pcPPS);
        ...
        {
            SEI::MessageList cSEIMessageList;
            SEI::read(m_pcHeaderSymbolReadIf, cSEIMessageList, m_pcParameterSetMngDecode);
        ...
        case NAL_UNIT_ACCESS_UNIT_DELIMITER: // just read, but ignore
        {
            ...
            cAUDelimiter.read (*m_pcHeaderSymbolReadIf);
        ...
        case NAL_UNIT_END_OF_SEQUENCE: // just read, but ignore
        {
            ...
            cEndOfSequence.read   (*m_pcHeaderSymbolReadIf);
        ...
        case NAL_UNIT_END_OF_STREAM: // just read, but ignore
        {
            ...
            cEndOfStream.read (*m_pcHeaderSymbolReadIf);
        ...
        case NAL_UNIT_FILLER_DATA: // just read, but ignore
        {
            ...
            cFillerData.read(*m_pcHeaderSymbolReadIf);
...
ErrVal H264AVCDecoder::getBaseLayerData(SliceHeader&      rcELSH,
                                        Frame*&           pcFrame,
                                        Frame*&           pcResidual,
                                        ...
                                        UInt              uiBaseLayerId)
{
    m_apcLayerDecoder[uiBaseLayerId]->getBaseLayerData(rcELSH,
                                                       pcFrame,
                                                       pcResidual,
                                                       pcMbDataCtrl,
                                                       rcResizeParameters);
...
ErrVal H264AVCDecoder::getBaseSliceHeader(SliceHeader*& rpcSliceHeader, UInt uiRefLayerDependencyId)
{
    m_apcLayerDecoder[uiRefLayerDependencyId]->getBaseSliceHeader(rpcSliceHeader);
    return Err::m_nOK;
}
