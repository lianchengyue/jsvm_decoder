...
{
    ROF(pcBitReadBuffer);
    CabaDecoder::init(pcBitReadBuffer);
    return Err::m_nOK;
}
...
ErrVal CabacReader::xRefFrame(MbDataAccess& rcMbDataAccess, UInt& ruiRefFrame, ListIdx eLstIdx, ParIdx8x8 eParIdx)
{
    ...
    CabaDecoder::getSymbol(ruiRefFrame, m_cRefPicCCModel.get(0, uiCtx));

    if (0 != ruiRefFrame)
    {
        CabaDecoder::getUnarySymbol(ruiRefFrame, &m_cRefPicCCModel.get(0, 4), 1);
        ruiRefFrame++;
    }
...
ErrVal CabacReader::xMotionPredFlag( Bool& bFlag, ListIdx eLstIdx)
{
    ...
    CabaDecoder::getSymbol(uiCode, m_cMotPredFlagCCModel.get(0, eLstIdx));
...
ErrVal CabacReader::blockModes(MbDataAccess& rcMbDataAccess)
{
    ...
    for(B8x8Idx c8x8Idx; c8x8Idx.isLegal(); c8x8Idx++)
    {
        ...
        {
            CabaDecoder::getSymbol(uiSymbol, m_cBlockTypeCCModel.get(0, 1));
            if(0 == uiSymbol)
            {
                CabaDecoder::getSymbol(uiSymbol, m_cBlockTypeCCModel.get(0, 3));
                if(0 != uiSymbol)
                {
                    CabaDecoder::getSymbol(uiSymbol, m_cBlockTypeCCModel.get(0, 4));
        ...
        {
            CabaDecoder::getSymbol(uiSymbol, m_cBlockTypeCCModel.get(1, 0));
            if(0 != uiSymbol)
            {
                CabaDecoder::getSymbol(uiSymbol, m_cBlockTypeCCModel.get(1, 1));
                if(0 != uiSymbol)
                {
                    CabaDecoder::getSymbol(uiSymbol, m_cBlockTypeCCModel.get(1, 2));
                    if(0 != uiSymbol)
                    {
                      CabaDecoder::getSymbol(uiSymbol, m_cBlockTypeCCModel.get(1, 3));
                      if(0 != uiSymbol)
                      {
                          uiBlockMode = 10;
                          CabaDecoder::getSymbol(uiSymbol, m_cBlockTypeCCModel.get(1, 3));
...
ErrVal CabacReader::resPredFlag(MbDataAccess& rcMbDataAccess)
{
    ...
    CabaDecoder::getSymbol(uiSymbol, m_cResPredFlagCCModel.get(0, uiCtx));
...
ErrVal CabacReader::mbMode(MbDataAccess& rcMbDataAccess)
{
    ...
    {
        CabaDecoder::getSymbol(act_sym, m_cMbTypeCCModel.get(0, rcMbDataAccess.getCtxMbIntra4x4()));

        if(0 != act_sym)
        {
            CabaDecoder::getTerminateBufferBit(act_sym);
            if(0 != act_sym)
            {
              act_sym = 25;
            }
            ...
            {
              CabaDecoder::getSymbol(act_sym, m_cMbTypeCCModel.get(0, 4));
              act_sym = 12* act_sym + 1;

              CabaDecoder::getSymbol(mod_sym, m_cMbTypeCCModel.get(0, 5));

              if(0 != mod_sym)
              {
                  CabaDecoder::getSymbol(mod_sym, m_cMbTypeCCModel.get(0, 6));
                  act_sym += ++mod_sym << 2;
              }

              CabaDecoder::getSymbol(mod_sym, m_cMbTypeCCModel.get(0, 7));
              act_sym += mod_sym << 1;

              CabaDecoder::getSymbol(mod_sym, m_cMbTypeCCModel.get(0, 8));
              act_sym += mod_sym;
            }
        }
        uiMbMode = act_sym;
    ...
    else
    {
        ...
        if(! rcMbDataAccess.getSH().isBSlice())
        {
          CabaDecoder::getSymbol(uiSymbol, m_cMbTypeCCModel.get(1, 4));
          if(0 != uiSymbol)
          {
              CabaDecoder::getSymbol(uiSymbol, m_cMbTypeCCModel.get(1, 7));
              uiMbMode = (0 != uiSymbol) ? 7 : 6;
          }
          else
          {
            CabaDecoder::getSymbol(uiSymbol, m_cMbTypeCCModel.get(1, 5));
            if(0 != uiSymbol)
            {
                CabaDecoder::getSymbol(uiSymbol, m_cMbTypeCCModel.get(1, 7));
            ...
            {
                CabaDecoder::getSymbol(uiSymbol, m_cMbTypeCCModel.get(1, 6));
        ...
        {
            CabaDecoder::getSymbol(uiSymbol, m_cMbTypeCCModel.get(2, rcMbDataAccess.getCtxMbType()));
            if(0 != uiSymbol)
            {
                CabaDecoder::getSymbol(uiSymbol, m_cMbTypeCCModel.get(2, 4));
                if(0 != uiSymbol)
                {
                    CabaDecoder::getSymbol(uiSymbol, m_cMbTypeCCModel.get(2, 5));
                    if(0 != uiSymbol)
                    {
                        uiMbMode = 12;
                        CabaDecoder::getSymbol(uiSymbol, m_cMbTypeCCModel.get(2, 6));
                        uiMbMode += uiSymbol << 3;
                        CabaDecoder::getSymbol(uiSymbol, m_cMbTypeCCModel.get(2, 6));
                        uiMbMode += uiSymbol << 2;
                        CabaDecoder::getSymbol(uiSymbol, m_cMbTypeCCModel.get(2, 6));
        ...
        {
            CabaDecoder::getTerminateBufferBit(uiSymbol);
            if(0 != uiSymbol)
            {
                uiMbMode += 24;
            }
            ...
            {
                CabaDecoder::getSymbol(uiSymbol, m_cMbTypeCCModel.get(1, 8));
                uiMbMode += (0 != uiSymbol) ? 12 : 0;

                CabaDecoder::getSymbol(uiSymbol, m_cMbTypeCCModel.get(1, 9));
                if(0 != uiSymbol)
                {
                    CabaDecoder::getSymbol(uiSymbol, m_cMbTypeCCModel.get(1, 9));
                ...
                CabaDecoder::getSymbol(uiSymbol, m_cMbTypeCCModel.get(1, 10));
                uiMbMode += uiSymbol << 1;
                CabaDecoder::getSymbol(uiSymbol, m_cMbTypeCCModel.get(1, 10));
                uiMbMode += uiSymbol;
            }
...
ErrVal CabacReader::xGetMvdComponent(Short& rsMvdComp, UInt uiAbsSum, UInt uiCtx)
{
    ...
    UInt uiSymbol;
    CabaDecoder::getSymbol(uiSymbol, m_cMvdCCModel.get(0, uiLocalCtx));

    ROTRS(0 == uiSymbol, Err::m_nOK)

    CabaDecoder::getExGolombMvd(uiSymbol, &m_cMvdCCModel.get(1, uiCtx), 3);
    uiSymbol++;

    UInt uiSign;
    CabaDecoder::getEpSymbol(uiSign);
...
{
    UInt uiSymbol;
    CabaDecoder::getSymbol(uiSymbol, m_cFieldFlagCCModel.get(0, rcMbDataAccess.getCtxFieldFlag()));
...
{
    UInt uiSymbol;
    CabaDecoder::getSymbol(uiSymbol, m_cChromaPredCCModel.get(0, rcMbDataAccess.getCtxChromaPredMode()));

    if(uiSymbol)
    {
      CabaDecoder::getUnaryMaxSymbol(uiSymbol, m_cChromaPredCCModel.get(0) + 3, 0, 2);
      uiSymbol++;
    }
...
ErrVal CabacReader::intraPredModeLuma(MbDataAccess& rcMbDataAccess, LumaIdx cIdx)
{
    ...
    CabaDecoder::getSymbol(uiSymbol, m_cIntraPredCCModel.get(0, 0));

    if(! uiSymbol)
    {
        CabaDecoder::getSymbol(uiSymbol, m_cIntraPredCCModel.get(0, 1));
...
ErrVal CabacReader::cbp(MbDataAccess& rcMbDataAccess, UInt uiStart, UInt uiStop)
{
    ...
    CabaDecoder::getSymbol(uiBit, m_cCbpCCModel.get(uiCtx, 3 - (a + b)));
    uiCbp = uiBit;
    ...
    if(rcMbDataAccess.getSH().getSPS().getChromaFormatIdc())
    {
        ...
        CabaDecoder::getSymbol(uiBit, m_cCbpCCModel.get(uiCtx, a + b));
...
ErrVal CabacReader::deltaQp(MbDataAccess& rcMbDataAccess)
{
    ...
    CabaDecoder::getSymbol(uiDQp, m_cDeltaQpCCModel.get(0, uiCtx));

    m_uiLastDQpNonZero = uiDQp;

    if(uiDQp)
    {
        CabaDecoder::getUnarySymbol(uiDQp, &m_cDeltaQpCCModel.get(0, 2), 1);
...
ErrVal CabacReader::xReadBCbp(MbDataAccess& rcMbDataAccess, Bool& rbCoded, ResidualMode eResidualMode, ChromaIdx cIdx, UInt uiStart, UInt uiStop)
{
    ...
    CabaDecoder::getSymbol(uiBit, m_cBCbpCCModel.get(type2ctx1[ eResidualMode ], uiCtx));
                                ...
                                UInt            uiStop)
{
    ...
    for(ui = uiStart; ui < uiStop; ui++) // if last coeff is reached, it has to be significant
    {
        ...
        CabaDecoder::getSymbol(uiSig, rcMapCCModel.get(type2ctx2[eResidualMode], ui));
        ...
        if(uiSig)
        {
            ...
            UInt uiLast;
            CabaDecoder::getSymbol(uiLast, rcLastCCModel.get(type2ctx2[eResidualMode], ui));
            if(uiLast)
            {
                break;
            }
    ...
    while((ui--) != uiStart)
    {
        ...
        if(uiCoeff)
        {
            ...
            CabaDecoder::getSymbol(uiCoeff, m_cOneCCModel.get(type2ctx1[eResidualMode], uiCtx));

            if(1 == uiCoeff)
            {
                uiCtx = gMin (c2,4);
                CabaDecoder::getExGolombLevel(uiCoeff, m_cAbsCCModel.get(type2ctx1[eResidualMode], uiCtx));
                uiCoeff += 2;
...
ErrVal CabacReader::transformSize8x8Flag(MbDataAccess& rcMbDataAccess)
{
    ...
    CabaDecoder::getSymbol(uiSymbol, m_cTransSizeCCModel.get(0, uiCtx));
                                     ...
                                     UInt          uiStop)
{
...

    {
        ...
        for(ui = uiStart; ui < uiStop; ui++) // if last coeff is reached, it has to be significant
        {
            UInt uiSig;
            //--- read significance symbol ---
            CabaDecoder::getSymbol(uiSig, pcMapCCModel[pos2ctx_map[ui]]);
            ...
            if(uiSig)
            {
                ...
                CabaDecoder::getSymbol(uiLast, pcLastCCModel[pos2ctx_last8x8[ui]]);
    ...
    while((ui--) != uiStart)
    {
        ...
        if(uiCoeff)
        {
            ...
            CabaDecoder::getSymbol(uiCoeff, m_cOneCCModel.get(uiCtxOffset, uiCtx));

            if(1 == uiCoeff)
            {
                uiCtx = gMin(c2,4);
                CabaDecoder::getExGolombLevel(uiCoeff, m_cAbsCCModel.get(uiCtxOffset, uiCtx));
                uiCoeff += 2;
