...
ErrVal DPBUnit::xCreateData(const SequenceParameterSet& rcSPS,
                            Bool bBaseLayer)
{
  ROT(m_pcFrame);
  ...
  if(bBaseLayer)
  {
      ...
      m_pcMbDataCtrlBaseLayer->init(rcSPS);
  }
  return Err::m_nOK;
}

ErrVal DPBUnit::xDeleteData()
{
    if(m_pcFrame)
    {
        m_pcFrame->uninit();
    ...
    {
        m_pcMbDataCtrlBaseLayer->uninit();
...
ErrVal DPBUnit::dump(UInt uiNumber, Bool bLineBefore, Bool bLineAfter)
{
    ROF(m_ePicStatus);
    Int iWidth  = 60;
    ...
    for(Int iFld = iMinFld; iFld < iMaxFld; iFld++)
    {
      ...
      if (!m_abNeededForReference[iFld])
          printf(" --");
      else if (m_abLongTerm[iFld])
          printf(" LT");
      else
          printf(" ST");              // 12 (+3)
...
Bool DPBUnit::isUsedForRef() const
{
    ROTRS (m_abNeededForReference[0], true);
    ROTRS (m_abNeededForReference[1], true);
    return false;
...
Bool DPBUnit::isShortTermUnit() const
{
...
Bool DPBUnit::isLongTermUnit() const
{
    ROTRS(m_abNeededForReference[0] && m_abLongTerm[0], true);
    ROTRS(m_abNeededForReference[1] && m_abLongTerm[1], true);
    return false;
...
Int DPBUnit::getMaxPoc(Bool bPocMode0) const
{
...
Int DPBUnit::getFrameNumWrap(UInt uiCurrFrameNum, UInt uiMaxFrameNum) const
{
    ROFRS(m_uiFrameNum>uiCurrFrameNum, (Int)m_uiFrameNum);
    return Int(m_uiFrameNum - uiMaxFrameNum);
}

Bool DPBUnit::isShortTermRef(PicType ePicType) const
{
...
Bool DPBUnit::isLongTermRef(PicType ePicType) const
{
    ROFRS(ePicType == FRAME, m_abNeededForReference[ePicType-1] && m_abLongTerm[ePicType-1]);
    return isLongTermRef(TOP_FIELD) && isLongTermRef(BOT_FIELD);
}

Bool DPBUnit::isRefPic(PicType ePicType) const
{
    ROFRS(ePicType == FRAME, m_abNeededForReference[ePicType-1]);
    return isRefPic(TOP_FIELD) && isRefPic(BOT_FIELD);
}

ErrVal DPBUnit::setNonExisting(UInt uiFrameNum, Int iTopFieldPoc, Int iBotFieldPoc)
{
    uninit();
    m_ePicStatus              = FRAME;
...
ErrVal DPBUnit::output(PicBufferList& rcPicBufferInputList, PicBufferList& rcPicBufferOutputList, PicBufferList& rcPicBufferUnusedList)
{
    ...
    m_pcFrame->store(pcPicBuffer, m_ePicStatus);
...
ErrVal DPBUnit::markUnusedForRef(PicType ePicType, Bool bRemoveOutputFlag)
{
...
ErrVal DPBUnit::markLongTerm(PicType ePicType, Int iLongTermFrameIdx)
{
    if((ePicType & TOP_FIELD) == TOP_FIELD)
    {
...
ErrVal DPBUnit::decreasePoc(Int iMMCO5Poc)
{
    ROF(m_ePicStatus);
...
ErrVal DPBUnit::checkStatus(Int iMaxLongTermFrameIdx)
{
    ROF( m_ePicStatus);
...
ErrVal CurrDPBUnit::create(CurrDPBUnit*& rpcCurrDPBUnit,
                           YuvBufferCtrl& rcYuvBufferCtrl,
                           const SequenceParameterSet& rcSPS,
                           ...
                           Bool bBaseLayer)
{
    ...
    rpcCurrDPBUnit->xCreateData(rcSPS, bCreateRefBasePicBuffer, bBaseLayer);
    return Err::m_nOK;
}

ErrVal CurrDPBUnit::destroy()
{
    xDeleteData();
    delete this;
    return Err::m_nOK;
...
ErrVal CurrDPBUnit::xCreateData(const SequenceParameterSet& rcSPS,
                                Bool bCreateRefBasePicBuffer,
                                Bool bBaseLayer)
{
    ROT (m_pcControlData);
    ...
    DPBUnit::xCreateData(rcSPS, bBaseLayer);
    ...
    //===== create and init control data =====
    {
        ...
        m_pcControlData ->setMbDataCtrl(pcMbDataCtrl);
    }
    ...
    if(bCreateRefBasePicBuffer)
    {
        ...
        m_pcRefBasePicFrame->init();
    }
    return Err::m_nOK;
}

ErrVal CurrDPBUnit::xDeleteData()
{
    DPBUnit::xDeleteData();
    if(m_pcControlData)
    {
        ...
        {
            m_pcControlData->getMbDataCtrl()->uninit();
    ...
    {
        m_pcRefBasePicFrame->uninit();
        m_pcRefBasePicFrame->destroy();
        m_pcRefBasePicFrame = 0;
    }
    return Err::m_nOK;
}

ErrVal CurrDPBUnit::init(SliceHeader& rcSliceHeader)
{
    ...
    //===== init frame =====
    m_pcFrame->addFrameFieldBuffer();
    m_pcFrame->setPoc(rcSliceHeader);
    //===== init reference base picture frame =====
    if(m_pcRefBasePicFrame)
    {
        m_pcRefBasePicFrame->addFrameFieldBuffer();
        m_pcRefBasePicFrame->setPoc(rcSliceHeader);
    }
    //===== init control data =====
    m_pcControlData->clear();
    m_pcControlData->setSliceHeader(&rcSliceHeader);
    m_pcControlData->getMbDataCtrl()->reset();
    m_pcControlData->getMbDataCtrl()->clear();
    return Err::m_nOK;
}

ErrVal CurrDPBUnit::reinit(SliceHeader& rcSliceHeader, Bool bNewLayerRepresentation)
{
    ROF(m_bInUse);
...
ErrVal CurrDPBUnit::resetMMCO5(SliceHeader& rcSliceHeader)
{
...
ErrVal CurrDPBUnit::setComplete(CurrDPBUnit& rcILPredUnit, Bool bDependencyRepresentationFinished)
{
    ROF(m_bInUse && !m_bCompleted);
    //===== copy base layer macroblock data =====
    if(m_pcMbDataCtrlBaseLayer && !m_uiQualityId)
    {
        m_pcMbDataCtrlBaseLayer->copyMotion(*m_pcControlData->getMbDataCtrl(), m_ePicStatus);
    }
    //===== set complete =====
    m_bCompleted = true;
    ...
    else
    {
        ...
        m_pcControlData->setMbDataCtrl(rcILPredUnit.m_pcControlData->getMbDataCtrl());
        rcILPredUnit.m_pcControlData->setMbDataCtrl(pcMbDataCtrl);
        rcILPredUnit.m_pcControlData->setSliceHeader(pcSliceHeader);
    }
    return Err::m_nOK;
}

ErrVal CurrDPBUnit::store(DPBUnit& rcDPBUnit, Bool bRefBasePic)
{
    //===== store picture =====
    if(rcDPBUnit.m_ePicStatus)
    {
        xStore2ndField(rcDPBUnit, bRefBasePic);
    }
    else
    {
        xStoreFrame(rcDPBUnit, bRefBasePic);
    }
    //===== fill image border =====
    {
        SliceHeader* pcSliceHeader = m_pcControlData->getSliceHeader();
        ROF (pcSliceHeader);
        rcDPBUnit.m_rcYuvBufferCtrl.initMb();
        rcDPBUnit.m_pcFrame->extendFrame(0, rcDPBUnit.m_ePicStatus, pcSliceHeader->getSPS().getFrameMbsOnlyFlag());
    }
    return Err::m_nOK;
}

ErrVal CurrDPBUnit::xStoreFrame(DPBUnit& rcDPBUnit, Bool bRefBasePic)
{
    ROF(m_bInUse && m_bCompleted);
    //===== safety checks =====
...
ErrVal CurrDPBUnit::xStore2ndField(DPBUnit& rcDPBUnit, Bool bRefBasePic)
{
    ROF(m_bInUse && m_bCompleted);
    //===== safety checks =====
    ...
    //===== update frame =====
    {
        ...
        ROF (pcSliceHeader);
        ROF (pcFrame);
        rcDPBUnit.m_pcFrame->copy(pcFrame, m_ePicStatus);
    ...
    {
        ROF (m_pcMbDataCtrlBaseLayer);
        rcDPBUnit.m_pcMbDataCtrlBaseLayer->copyMotion(*m_pcMbDataCtrlBaseLayer, m_ePicStatus);
    }
    return Err::m_nOK;
}

ErrVal CurrDPBUnit::uninit()
{
    ROF (m_bInUse && m_bCompleted);
    DPBUnit::uninit();
    m_bInUse            = false;
...
ErrVal DecodedPicBuffer::create(DecodedPicBuffer*& rpcDecodedPicBuffer)
{
    ROF((rpcDecodedPicBuffer = new DecodedPicBuffer()));
    return Err::m_nOK;
}

ErrVal DecodedPicBuffer::destroy()
{
    uninit();
    delete this;
    return Err::m_nOK;
...
ErrVal DecodedPicBuffer::init(YuvBufferCtrl* pcYuvBufferCtrl, UInt uiDependencyId)
{
    ROT(m_bInitDone);
...
ErrVal DecodedPicBuffer::uninit()
{
    xDeleteData();
    m_bInitDone = false;
    m_bInitBufferDone = false;
...
Bool DecodedPicBuffer::xNewBufferDimension(const SequenceParameterSet& rcSPS)
{
...
ErrVal DecodedPicBuffer::xInitBuffer(const SliceHeader& rcSliceHeader)
{
    ROF(m_bInitDone);
    ...
    xCreateData(rcSliceHeader.getSPS(), m_uiDPBSizeInFrames, m_uiDependencyId == 0);
...
ErrVal DecodedPicBuffer::xCreateData(const SequenceParameterSet& rcSPS,
                                     ...
                                     Bool bBaseLayer)
{
    ...
    while(uiDPBSizeInFrames--)
    {
        ...
        DPBUnit::create(pcDPBUnit, *m_pcYuvBufferCtrl, rcSPS, bBaseLayer);
        m_cFreeDPBUnitList.pushBack(pcDPBUnit);
    }
    //===== create current DPB units =====
    CurrDPBUnit::create(m_pcCurrDPBUnit, *m_pcYuvBufferCtrl, rcSPS, true,  bBaseLayer);
    CurrDPBUnit::create(m_pcCurrDPBUnitILPred, *m_pcYuvBufferCtrl, rcSPS, false, false);
    return Err::m_nOK;
}

ErrVal DecodedPicBuffer::xDeleteData()
{
    ...
    {
        m_pcCurrDPBUnitILPred->destroy();
        m_pcCurrDPBUnitILPred = 0;
    }
    return Err::m_nOK;
}

ErrVal DecodedPicBuffer::initCurrDPBUnit(CurrDPBUnit*& rpcCurrDPBUnit,
                                         ...
                                         Bool bFirstSliceInLayerRepresentation)
{
    ...
    if(bFirstSliceInDependencyRepresentation)
    {
        ...
        if(rcSliceHeader.getIdrFlag())
        {
            ...
            {
                xMarkAllUnusedForRef(rcSliceHeader.getNoOutputOfPriorPicsFlag(), &rcUnusedList);
                xBumpingOutput(rcOutputList, rcUnusedList, true);
                xCheckBufferStatus();
            }
            xInitBuffer(rcSliceHeader);
        }
        //--- check status ---
        ...
        xCheckGapsInFrameNum(rcSliceHeader, rcPocCalculator, rcOutputList, rcUnusedList);
        //--- initialize POC and DPB unit ---
        rcPocCalculator.calculatePoc(rcSliceHeader);
        m_pcCurrDPBUnit->init(rcSliceHeader);
    }
    else
    {
      //--- check status ---
      ...
      //--- initialize POC and DPB unit ---
      rcPocCalculator.calculatePoc(rcSliceHeader);
      m_pcCurrDPBUnit->reinit(rcSliceHeader, bFirstSliceInLayerRepresentation);
    }
    rpcCurrDPBUnit = m_pcCurrDPBUnit;
    return Err::m_nOK;
}

ErrVal DecodedPicBuffer::storeCurrDPBUnit(Bool bDependencyRepresentationFinished)
{
    ROF (m_bInitDone && m_bInitBufferDone);
    ROF (m_pcCurrDPBUnit->inCurrentUse ());
    m_pcCurrDPBUnit->setComplete  (*m_pcCurrDPBUnitILPred, bDependencyRepresentationFinished);
    return Err::m_nOK;
}

ErrVal DecodedPicBuffer::updateBuffer(PocCalculator& rcPocCalculator, PicBufferList& rcOutputList, PicBufferList& rcUnusedList)
{
    ROF   (m_bInitDone);
    ...
    {
        m_pcCurrDPBUnitILPred->uninit();
    }
    //===== update DPB and check whether current DPB unit was correctly stored =====
    xUpdateAndStoreCurrentPic(rcPocCalculator, rcOutputList, rcUnusedList);
    ROF   (m_pcCurrDPBUnit->isUninitialized());
    return Err::m_nOK;
}

ErrVal DecodedPicBuffer::finish(PicBufferList& rcOutputList, PicBufferList& rcUnusedList)
{
    ROF   (m_bInitDone);
    ROFRS (m_bInitBufferDone, Err::m_nOK);
    ROF   (m_pcCurrDPBUnit->isUninitialized());
    ROF   (m_pcCurrDPBUnitILPred->isUninitialized());
    xMarkAllUnusedForRef();
    xBumpingOutput(rcOutputList, rcUnusedList, true);
    xCheckBufferStatus();
...
ErrVal DecodedPicBuffer::setPrdRefLists(CurrDPBUnit* pcCurrDPBUnit)
{
    ...
    xCreateInitialRefPicLists(cRefPicEntryList0, cRefPicEntryList1, rcSliceHeader);
    xRefPicListModification(cRefPicEntryList0, rcSliceHeader, LIST_0);
    xSetReferencePictureList(rcRefPicList0, cRefPicEntryList0, rcSliceHeader.getNumRefIdxL0Active());
    xDumpRefPicList(rcRefPicList0, LIST_0, "final");
    ROFRS (rcSliceHeader.isBSlice(), Err::m_nOK);
    //===== set list 1 =====
    xRefPicListModification(cRefPicEntryList1, rcSliceHeader, LIST_1);
    xSetReferencePictureList(rcRefPicList1, cRefPicEntryList1, rcSliceHeader.getNumRefIdxL1Active());
    xDumpRefPicList(rcRefPicList1, LIST_1, "final");
    if(rcSliceHeader.isH264AVCCompatible())
    {
        xSetMbDataCtrlEntry0(pcMbDataCtrl0, cRefPicEntryList1);
        m_pcCurrDPBUnit->getCtrlData().setMbDataCtrl0L1(pcMbDataCtrl0);
    }
    return Err::m_nOK;
}

CurrDPBUnit* DecodedPicBuffer::getILPredDPBUnit()
{
...
Bool DecodedPicBuffer::xIs2ndFieldOfCompFieldPair(const SliceHeader& rcSliceHeader)
{
...
Bool DecodedPicBuffer::xIs2ndFieldOfCompFieldPair(Bool bRefBasePic)
{
...
ErrVal DecodedPicBuffer::xInsertNonExistingFrame(const SliceHeader* pcSliceHeader, UInt uiFrameNum)
{
    //===== determine POC =====
    ...
    DPBUnit* pcDPBUnit = m_cFreeDPBUnitList.popFront();
    pcDPBUnit->setNonExisting(uiFrameNum, iTopFieldPoc, iBotFieldPoc);
    m_cUsedDPBUnitList.push_back(pcDPBUnit);
    return Err::m_nOK;
}

ErrVal DecodedPicBuffer::xInsertCurrentInNewBuffer(DPBUnit*& rpcStoredDPBUnit, Bool bRefBasePic)
{
    ROF (m_uiMaxNumRefFrames > (bRefBasePic ? 1U : 0U));
    ROF (m_pcCurrDPBUnit->isCompleted());
    ROT (m_cFreeDPBUnitList.empty());
    ROF((rpcStoredDPBUnit = m_cFreeDPBUnitList.popFront()));
    m_pcCurrDPBUnit->store(*rpcStoredDPBUnit, bRefBasePic);
...
ErrVal DecodedPicBuffer::xCheckGapsInFrameNum(const SliceHeader& rcSliceHeader,
                                              PocCalculator& rcPocCalculator,
                                              PicBufferList& rcOutputList,
                                              PicBufferList& rcUnusedList)
{
    ...
    while(uiMissingFrames--)
    {
        ...
        if(pcNonExSliceHeader)
        {
            ...
            rcPocCalculator.calculatePoc(*pcNonExSliceHeader);
        }
        xSlidingWindow(uiFrameNum);
        xInsertNonExistingFrame(pcNonExSliceHeader, uiFrameNum);
        xBumpingOutput(rcOutputList, rcUnusedList);
        xCheckBufferStatus();
    }
    m_pcLastDPBUnit = 0;
    m_pcLastDPBUnitRefBasePic = 0;
...
ErrVal DecodedPicBuffer::xUpdateAndStoreCurrentPic(PocCalculator& rcPocCalculator, PicBufferList& rcOutputList, PicBufferList& rcUnusedList)
{
    ROF(m_bInitDone && m_bInitBufferDone);
    ROF(m_pcCurrDPBUnit->isCompleted());
    ...
    {
        m_pcCurrDPBUnit->uninit();
        xCheckBufferStatus();
        xDumpDPB("after update (non-output and non-ref pic)");
        m_pcLastDPBUnit = 0;
        m_pcLastDPBUnitRefBasePic = 0;
        return Err::m_nOK;
    }
    ...
    if(!bIsRefPic && bIs2ndFld)
    {
        ...
        m_pcCurrDPBUnit->store(*m_pcLastDPBUnit);
        m_pcCurrDPBUnit->uninit();
        xBumpingOutput(rcOutputList, rcUnusedList);
        xCheckBufferStatus();
        xDumpDPB("after update (non-ref 2nd field)");
        return Err::m_nOK;
    }
    ...
    if(!bIsRefPic)
    {
        xInsertCurrentInNewBuffer(m_pcLastDPBUnit);
        m_pcCurrDPBUnit->uninit();
        if(!pcSliceHeader->getFieldPicFlag())
        {
            xBumpingOutput(rcOutputList, rcUnusedList);
            xCheckBufferStatus();
            xDumpDPB("after update (non-ref frame)");
        }
        else
        {
            xDumpDPB("after update (non-ref 1st field)");
        }
        m_pcLastDPBUnitRefBasePic = 0;
        return  Err::m_nOK;
    }
    ...
    if(bIsIDR)
    {
        ...
        if(pcSliceHeader->getLongTermReferenceFlag())
        {
            ...
            m_pcCurrDPBUnit->markLongTerm (pcSliceHeader->getPicType(), m_iMaxLongTermFrameIdx);
        }
        xInsertCurrentInNewBuffer(m_pcLastDPBUnit);
        if(pcSliceHeader->getStoreRefBasePicFlag())
        {
            xInsertCurrentInNewBuffer(m_pcLastDPBUnitRefBasePic, true);
        }
        m_pcCurrDPBUnit->uninit();
        xCheckBufferStatus();
        xDumpDPB("after update (IDR pic)");
        return  Err::m_nOK;
    }
    ...
    {
        xMMCO(rcPocCalculator, *pcSliceHeader, bStoredAsLongTerm, true);
    ...
    {
        xMMCO(rcPocCalculator, *pcSliceHeader, bStoredAsLongTerm);
    }
    else if(!bIs2ndFld)
    {
        xSlidingWindow(uiCurrFrameNum);
    }
    //----- store pictures -----
    if(!bStoredAsLongTerm)
    {
      ...
      if(pcSliceHeader->getStoreRefBasePicFlag())
      {
          if(bIs2ndFldBase)
          {
              m_pcCurrDPBUnit->store(*m_pcLastDPBUnitRefBasePic, true);
          }
          else
          {
              xSlidingWindow(uiCurrFrameNum);
    ...
    xBumpingOutput(rcOutputList, rcUnusedList);
    xCheckBufferStatus();
    xDumpDPB("after update (non-IDR ref pic)");
    return Err::m_nOK;
}

ErrVal DecodedPicBuffer::xCheckBufferStatus()
{
    ROF(m_bInitDone && m_bInitBufferDone);
    ...
    for(; iter != end; iter++)
    {
        (*iter)->checkStatus(m_iMaxLongTermFrameIdx);
        if(!(*iter)->isRequired ())
        {
            uiNumNonRequired++;
        }
...
ErrVal DecodedPicBuffer::xBumpingOutput(PicBufferList& rcOutputList, PicBufferList& rcUnusedList, Bool bOutputAll)
{
    ROF(m_bInitDone && m_bInitBufferDone);
    ...
    while(uiNumRequiredRef + uiNumRequiredNonRef > m_uiDPBSizeInFrames || (bOutputAll && uiNumRequiredNonRef))
    {
        ...
        pcDPBUnitToOutput->output(m_cPicBufferList, rcOutputList, rcUnusedList);
...
ErrVal DecodedPicBuffer::xMarkAllUnusedForRef(Bool bRemoveOutputFlag, PicBufferList* pcUnusedList)
{
    ...
    for(; iter != end; iter++)
    {
        (*iter)->markUnusedForRef(FRAME, bRemoveOutputFlag);
        if(bRemoveOutputFlag)
        {
...
ErrVal DecodedPicBuffer::xSlidingWindow(UInt uiCurrFrameNum)
{
    ROF(m_bInitDone && m_bInitBufferDone);
    ...
    pcMinFrameNumUnit->markUnusedForRef(FRAME);
    return Err::m_nOK;
}

ErrVal DecodedPicBuffer::xMMCO(PocCalculator& rcPocCalculator, SliceHeader& rcSliceHeader, Bool& rbMMCO6, Bool bRefBasePic)
{
    Mmco  eMmcoOp;
    UInt  uiVal1, uiVal2;
    UInt  uiCurrFrameNum = rcSliceHeader.getFrameNum();
    ...
    while(MMCO_END != (eMmcoOp = rcMmcoBuffer.get(iIndex++).getCommand(uiVal1, uiVal2)))
    {
        ...
        case MMCO_SHORT_TERM_UNUSED:
            ROT (bMMCO5);
            xMarkShortTermUnused(uiCurrFrameNum, eCurrPicType, uiVal1, bRefBasePic);
            bMMCO123  = true;
            break;
        case MMCO_LONG_TERM_UNUSED:
            ROT (bMMCO5);
            ROT (rbMMCO6 && iMMCO6LTFrmIdx == (Int)(eCurrPicType == FRAME ? uiVal1 : (uiVal1 >> 1)));
            xMarkLongTermUnused(eCurrPicType, uiVal1, bRefBasePic);
            bMMCO123  = true;
            break;
        case MMCO_ASSIGN_LONG_TERM:
            ...
            xAssignLongTermIndex(uiCurrFrameNum, eCurrPicType, uiVal1, uiVal2);
            bMMCO123  = true;
            break;
        ...
        case MMCO_RESET:
            ...
            ROT (bMMCO5);
            ROT (rbMMCO6);
            xReset(rcPocCalculator, rcSliceHeader);
            bMMCO5  = true;
            break;
        case MMCO_SET_LONG_TERM:
            ...
            xStoreCurrentLongTerm(uiVal1, rcSliceHeader.getStoreRefBasePicFlag());
            rbMMCO6 = true;
...
ErrVal DecodedPicBuffer::xMarkShortTermUnused(UInt uiCurrFrameNum, PicType eCurrPicType, UInt uiPicNumDiff, Bool bRefBasePic)
{
    ...
    pcDPBUnit->markUnusedForRef(ePicTypeX);
    return Err::m_nOK;
}

ErrVal DecodedPicBuffer::xMarkLongTermUnused(PicType eCurrPicType, UInt uiLongTermPicNum, Bool bRefBasePic)
{
...
ErrVal DecodedPicBuffer::xAssignLongTermIndex(UInt uiCurrFrameNum, PicType eCurrPicType, UInt uiPicNumDiff, UInt uiLongTermFrameIndex)
{
    ...
    for(; iter != end; iter++)
    {
        if((*iter)->isLongTermUnit() &&
           ...
           (*iter) != pcDPBUnit)
        {
            (*iter)->markUnusedForRef(FRAME);
        }
    }
    //===== mark pictures =====
    ROF (pcDPBUnit);
    pcDPBUnit->markLongTerm(ePicTypeX, iLongTermFrameIdx);
    ROFRS (pcDPBUnitBase,  Err::m_nOK);
    pcDPBUnitBase->markLongTerm(ePicTypeX, iLongTermFrameIdx);
    return  Err::m_nOK;
}

ErrVal DecodedPicBuffer::xSetMaxLongTermIndex(UInt uiMaxLongTermFrameIdxPlus1)
{
...
ErrVal DecodedPicBuffer::xReset(PocCalculator& rcPocCalculator, SliceHeader& rcSliceHeader)
{
    ...
    rcPocCalculator.resetMMCO5(rcSliceHeader);
    //===== update parameters in current DPB unit =====
    ROF (m_pcCurrDPBUnit->isCompleted());
    m_pcCurrDPBUnit->resetMMCO5(rcSliceHeader);
    return Err::m_nOK;
}

ErrVal DecodedPicBuffer::xStoreCurrentLongTerm(UInt uiLongTermFrameIndex, Bool bStoreRefBasePic)
{
    Int  iLongTermFrameIdx  = (Int)uiLongTermFrameIndex;
    ...
    m_pcCurrDPBUnit->markLongTerm(m_pcCurrDPBUnit->getPicStatus(), uiLongTermFrameIndex);
...
Bool DecodedPicBuffer::xExistsRefBasePicShortTerm(UInt uiFrameNum)
{
    DPBUnitList::iterator iter = m_cUsedDPBUnitList.begin();
    DPBUnitList::iterator end  = m_cUsedDPBUnitList.end();
    for(; iter != end; iter++)
...
Bool DecodedPicBuffer::xExistsRefBasePicLongTerm (Int iLongTermFrameIdx)
{
    DPBUnitList::iterator iter = m_cUsedDPBUnitList.begin();
    DPBUnitList::iterator end  = m_cUsedDPBUnitList.end();
    for(; iter != end; iter++)
...
Bool DecodedPicBuffer::xIsAvailableForRefLists(const DPBUnit* pcDPBUnit, Bool bFieldPicture, Bool bExcludeNonExisting, Bool bUseRefBasePic)
{
    AOF  (pcDPBUnit);
...
ErrVal DecodedPicBuffer::xCreateOrderedDPBUnitLists(DPBUnitList& rcOrderedShortTermList0,
...
DPBUnit* DecodedPicBuffer::xGetDPBEntry(DPBUnitList& rcDPBUnitList, UInt uiIndex)
{
...
ErrVal DecodedPicBuffer::xGetRefPicEntryList(RefPicEntryList& rcRefPicEntryList, DPBUnitList& rcDPBUnitList, PicType eCurrPicType)
{
    ROF(eCurrPicType);
...
ErrVal DecodedPicBuffer::xCreateInitialRefPicLists(RefPicEntryList& rcRefPicEntryList0,
                                                   ...
                                                   SliceHeader&  rcSliceHeader)
{
    ...
    xCreateOrderedDPBUnitLists(cShortTermDPBUnitList0, cShortTermDPBUnitList1, cLongTermDPBUnitList, bShortTermListsIdentical, rcSliceHeader);
    //===== create reference picture entry list 0 =====
    xGetRefPicEntryList(rcRefPicEntryList0, cShortTermDPBUnitList0, ePicType);
    xGetRefPicEntryList(cLongTermList,      cLongTermDPBUnitList,   ePicType);
    rcRefPicEntryList0 += cLongTermList;
    ROT  (rcRefPicEntryList0.empty());
    ROFRS(bBSlice, Err::m_nOK);
    //===== create reference picture entry list 1 =====
    xGetRefPicEntryList(rcRefPicEntryList1, cShortTermDPBUnitList1, ePicType);
...
ErrVal DecodedPicBuffer::xModifyRefPicList(RefPicEntryList& rcRefPicEntryList, UInt& ruiRefIdx, Bool bLongTerm, PicType ePicType, UInt uiFrameNumOrLTIdx)
{
...
ErrVal DecodedPicBuffer::xRefPicListModification(RefPicEntryList& rcRefPicEntryList, SliceHeader& rcSliceHeader, ListIdx eListIdx)
{
    ...
    while(RPLR_END != (eCommand = rcRefPicListReordering.get(uiCommandIndex++).getCommand(uiIdentifier)))
    {
        if(eCommand == RPLR_NEG || eCommand == RPLR_POS)
        {
            ...
            xModifyRefPicList(rcRefPicEntryList, uiRefIdx, false, ePicTypeX, uiFrameNumX);
        }
        else
        {
            ...
            xModifyRefPicList(rcRefPicEntryList, uiRefIdx, true,  ePicTypeX, uiLTIndexX);
        }
    }
    return Err::m_nOK;
}

ErrVal DecodedPicBuffer::xSetReferencePictureList(RefFrameList& rcRefFrameList, RefPicEntryList& rcRefPicEntryList, UInt uiNumActiveEntries)
{
    ...
    for(UInt uiRefIdx = 0; uiRefIdx < uiNumActiveEntries; uiRefIdx++, iter++)
    {
        ...
        rcRefFrameList.add(pcPic);
    }
    return Err::m_nOK;
}

ErrVal DecodedPicBuffer::xSetMbDataCtrlEntry0(MbDataCtrl*& rpcMbDataCtrl, RefPicEntryList& rcRefPicEntryList)
{
...
ErrVal DecodedPicBuffer::xDumpDPB(const Char* pcString)
{
    ...
    {
        (*iter)->dump(uiIdx, uiIdx == 0, true);
    }
    printf("\n");
    return Err::m_nOK;
}

ErrVal DecodedPicBuffer::xDumpRefPicList(RefFrameList& rcRefFrameList, ListIdx eListIdx, const Char* pcString)
{
    ROFRS(m_bDebugOutput, Err::m_nOK);
...
ErrVal LayerDecoder::create(LayerDecoder*& rpcLayerDecoder)
{
...
ErrVal LayerDecoder::init(UInt                   uiDependencyId,
...
ErrVal LayerDecoder::processSliceData(PicBuffer*         pcPicBuffer,
                                      ...
                                      SliceDataNALUnit&  rcSliceDataNALUnit)
{
    ...
    xInitSlice(pcSliceHeader, pcPicBuffer, rcPicBufferOutputList, rcPicBufferUnusedList, rcSliceDataNALUnit);

    //===== parse, decode, and finish slice =====
    xParseSlice (*pcSliceHeader);
    xDecodeSlice(*pcSliceHeader, rcSliceDataNALUnit, bFirstSliceInLayerRepresentation);
    xFinishSlice(*pcSliceHeader, rcPicBufferOutputList, rcPicBufferUnusedList, rcSliceDataNALUnit, rcBinDataList);
    return Err::m_nOK;
}


ErrVal LayerDecoder::finishProcess(PicBufferList&  rcPicBufferOutputList,
                                   PicBufferList&  rcPicBufferUnusedList)
{
    ROF  (m_bInitialized);
    m_pcDecodedPictureBuffer->finish(rcPicBufferOutputList, rcPicBufferUnusedList);
    while(m_cSliceHeaderList.size())
    {
        SliceHeader* pcSliceHeader = m_cSliceHeaderList.popFront();
        delete pcSliceHeader;
...
ErrVal LayerDecoder::updateDPB(PicBufferList& rcPicBufferOutputList,
                               PicBufferList& rcPicBufferUnusedList)
{
    ROF (m_bInitialized);
    m_pcDecodedPictureBuffer->updateBuffer(*m_pcPocCalculator, rcPicBufferOutputList, rcPicBufferUnusedList);
    return Err::m_nOK;
}

ErrVal LayerDecoder::xInitSlice(SliceHeader*&     rpcSliceHeader,
                                ...
                                SliceDataNALUnit& rcSliceDataNalUnit)
{
    ...
    xReadSliceHeader(rpcSliceHeader, rcSliceDataNalUnit);
    xInitSliceHeader(*rpcSliceHeader);
    xInitSPS(*rpcSliceHeader);
    m_cSliceHeaderList.push_back(rpcSliceHeader);

    //===== init DPB unit =====
    xInitDPBUnit (*rpcSliceHeader, pcPicBuffer, rcPicBufferOutputList, rcPicBufferUnusedList);

    //===== init resize parameters =====  ---> write new function xInitResizeParameters, which is somewhat nicer
    xInitESSandCroppingWindow (*rpcSliceHeader, *m_pcCurrDPBUnit->getCtrlData().getMbDataCtrl(), m_pcCurrDPBUnit->getCtrlData());
...
ErrVal LayerDecoder::xParseSlice(SliceHeader& rcSliceHeader)
{
    ROF(m_pcCurrDPBUnit);
    ...
    m_pcControlMng->initSliceForReading(rcSliceHeader);
    m_pcSliceReader->read(rcSliceHeader,
                          pcMbDataCtrl,
                          m_pacMbStatus,
                          m_uiFrameWidthInMb,
                          uiMbRead);
...
ErrVal LayerDecoder::xDecodeSlice(SliceHeader&  rcSliceHeader,
                                  ...
                                  Bool  bFirstSliceInLayerRepresentation)
{
    ...
    m_pcDecodedPictureBuffer->setPrdRefLists(m_pcCurrDPBUnit);
    ...
    //===== init base layer =====
    xInitBaseLayer(rcControlData, bFirstSliceInLayerRepresentation);

    //----- decoding -----
    m_pcControlMng->initSliceForDecoding(rcSliceHeader);

    if(rcSliceHeader.isMbaffFrame())
    {
        m_pcSliceDecoder->decodeMbAff(rcSliceHeader,
                                      pcMbDataCtrl,
    ...
    {
        //走这里
        m_pcSliceDecoder->decode(rcSliceHeader,
                                 pcMbDataCtrl,
                                 rcControlData.getBaseLayerCtrl(),
                                 pcRecFrame,
                                 m_pcResidualLF,
    ...
    printf("  %s %4d (LId%2d, TL%2d, QL%2d, %s-%c, BId%2d, AP%2d, QP%3d)\n",
            ...
            rcSliceHeader.getDependencyId         (),                            //LId
            rcSliceHeader.getTemporalId           (),                            //TL
            rcSliceHeader.getQualityId            (),                            //QL
            rcSliceHeader.isH264AVCCompatible     () ? "AVC" : "SVC",
            rcSliceHeader.getSliceType            () == I_SLICE ? 'I' :
            rcSliceHeader.getSliceType            () == P_SLICE ? 'P' : 'B',
            rcSliceHeader.getRefLayerDQId         (),                            //BID
            rcSliceHeader.getAdaptiveBaseModeFlag () ? 1 : 0,                    //AP
            rcSliceHeader.getSliceQp              ());                          //QP
    return Err::m_nOK;
}

ErrVal LayerDecoder::xFinishLayerRepresentation(SliceHeader&            rcSliceHeader,
                                                PicBufferList&          rcPicBufferOutputList,
                                                PicBufferList&          rcPicBufferUnusedList,
                                                const SliceDataNALUnit& rcSliceDataNalUnit,
                                                BinDataList&            rcBinDataList)
{
    ...
    xCheckForMissingSlices(rcSliceDataNalUnit, rcPicBufferOutputList, rcPicBufferUnusedList);

    //===== determine loop filter QPs =====
    xSetLoopFilterQPs(rcSliceHeader, *pcMbDataCtrl);

#ifdef SHARP_AVC_REWRITE_OUTPUT
    //===== rewrite picture =====
    if(rcSliceDataNalUnit.isHighestRewriteLayer())
    {
      xRewritePicture(rcBinDataList, *pcMbDataCtrl);
    }
#else
    //===== loop filter =====
    if(bReconstructBaseRep)
    {
        ...
        pcFrame->copy(pcBaseRepFrame, ePicType);

        //----- loop-filtering and store in DPB as base representation -----
        if(bDeblockBaseRep)
        {
            m_pcLoopFilter->process(rcSliceHeader,
                                    pcBaseRepFrame,
                                    m_pcResidualLF,
                                    pcMbDataCtrl,
    ...
    m_pcILPrediction->copy(pcFrame, ePicType);
    if(bReconstructAll)
    {
        m_pcLoopFilter->process(rcSliceHeader,
                                pcFrame,
    ...
    //===== store picture =====
    m_pcDecodedPictureBuffer->storeCurrDPBUnit(bDepRepFinished);
    if(bDPBUpdate)
    {
        m_pcH264AVCDecoder->updateDPB(m_uiDependencyId, rcPicBufferOutputList, rcPicBufferUnusedList);
    }
...
ErrVal LayerDecoder::xFinishSlice(SliceHeader&            rcSliceHeader,
                                  PicBufferList&          rcPicBufferOutputList,
                                  PicBufferList&          rcPicBufferUnusedList,
                                  const SliceDataNALUnit& rcSliceDataNalUnit,
                                  BinDataList&            rcBinDataList)
{
    ...
    xFinishLayerRepresentation(rcSliceHeader, rcPicBufferOutputList, rcPicBufferUnusedList, rcSliceDataNalUnit, rcBinDataList);
    m_bLayerRepresentationInitialized = false;
    m_bFirstILPredSliceInLayerRep     = false;
    m_bInterLayerPredLayerRep         = false;
    m_bRewritingLayerRep              = false;
...
ErrVal LayerDecoder::xCheckForMissingSlices(const SliceDataNALUnit& rcSliceDataNalUnit,
                                            PicBufferList&  rcPicBufferOutputList,
                                            PicBufferList&  rcPicBufferUnusedList)
{
    ...
    while(!cVirtualSkipSliceList.empty())
    {
        ...
        xInitDPBUnit(*pcSliceHeader, pcPicBuffer, rcPicBufferOutputList, rcPicBufferUnusedList);
        xInitESSandCroppingWindow(*pcSliceHeader, *m_pcCurrDPBUnit->getCtrlData().getMbDataCtrl(), m_pcCurrDPBUnit->getCtrlData());

        //===== parse and decode virtual slice =====
        xParseSlice(*pcSliceHeader);
        xDecodeSlice(*pcSliceHeader, rcSliceDataNalUnit, false);
    }

    return Err::m_nOK;
}

ErrVal LayerDecoder::xSetLoopFilterQPs(SliceHeader& rcSliceHeader, MbDataCtrl& rcMbDataCtrl)
{
    ...
    for(Int iSliceGroupId = rcFMO.getFirstSliceGroupId(); !rcFMO.SliceGroupCompletelyCoded(iSliceGroupId); iSliceGroupId = rcFMO.getNextSliceGroupId(iSliceGroupId))
    {
        ...
        for(UInt uiMbAddress = uiFirstMbInSliceGroup; uiMbAddress <= uiLastMbInSliceGroup; uiMbAddress = rcFMO.getNextMBNr(uiMbAddress))
        {
          ...
          rcMbDataCtrl.initMb(pcMbDataAccess, uiMbY, uiMbX);
...
#ifdef SHARP_AVC_REWRITE_OUTPUT
ErrVal LayerDecoder::xRewritePicture(BinDataList& rcBinDataList, MbDataCtrl& rcMbDataCtrl)
{
  ...
  m_pcRewriteEncoder->startPicture(rcSPS);
  ...
  for(Int iSliceGroupId = rcFMO.getFirstSliceGroupId(); ! rcFMO.SliceGroupCompletelyCoded(iSliceGroupId); iSliceGroupId = rcFMO.getNextSliceGroupId(iSliceGroupId))
  {
    ...
    for(UInt uiMbAddress = uiFirstMbInSliceGroup; uiMbAddress <= uiLastMbInSliceGroup; uiMbAddress = rcFMO.getNextMBNr(uiMbAddress))
    {
      ...
      rcMbDataCtrl       .initMb   (pcMbDataAccess, uiMbY, uiMbX);
      m_pcRewriteEncoder->rewriteMb(*pcMbDataAccess, bSendEOS);
    }
  }

  m_pcRewriteEncoder->finishPicture(rcBinDataList);
  return Err::m_nOK;
}
#endif

ErrVal LayerDecoder::xReadSliceHeader(SliceHeader*&      rpcSliceHeader,
                                      SliceDataNALUnit&  rcSliceDataNalUnit)
{
    ...
    m_pcNalUnitParser->initNalUnit(cBinDataAccessor, (Int)rcSliceDataNalUnit.getDQId());
    if(pcPrefixHeader)
    {
    ...
    rpcSliceHeader->read(*m_pcParameterSetMng, *m_pcHeaderSymbolReadIf);
...
ErrVal LayerDecoder::xInitSliceHeader(SliceHeader& rcSliceHeader)
{
    //===== check =====
    ROF(rcSliceHeader.getDependencyId() == m_uiDependencyId);
            ...
            && rcSliceHeader.getBasePredWeightTableFlag())
    {
        ...
        m_pcH264AVCDecoder->getBaseSliceHeader(pcBaseSliceHeader, rcSliceHeader.getRefLayerDependencyId());
...
#if 1
    // what is that -> is this really required ?????
...
ErrVal LayerDecoder::xInitSPS(const SliceHeader& rcSliceHeader)
{
    ROTRS(m_bSPSInitialized, Err::m_nOK);

    //===== init control manager =====
    m_pcControlMng->initSlice0(const_cast<SliceHeader*>(&rcSliceHeader));
    ...
    xDeleteData();
    xCreateData(rcSliceHeader.getSPS());
...
ErrVal LayerDecoder::xInitDPBUnit(SliceHeader&   rcSliceHeader,
                                  PicBuffer*     pcPicBuffer,
                                  PicBufferList& rcPicBufferOutputList,
                                  PicBufferList& rcPicBufferUnusedList)
{
    m_pcDecodedPictureBuffer->initCurrDPBUnit(m_pcCurrDPBUnit, pcPicBuffer, rcSliceHeader,
...
ErrVal LayerDecoder::getBaseSliceHeader(SliceHeader*& rpcSliceHeader)
{
    ROF(m_bInitialized);
...
ErrVal LayerDecoder::getBaseLayerData(SliceHeader&      rcELSH,
                                      Frame*&           pcFrame,
                                      Frame*&           pcResidual,
                                      ...
                                      ResizeParameters& rcResizeParameters)
{
    ...
    {
        m_apcFrameTemp[0]->copy(pcFrame, pcSliceHeader->getPicType());
        pcFrame = m_apcFrameTemp[0];
        m_pcLoopFilter->process(*pcSliceHeader, pcFrame, NULL, pcMbDataCtrl,
...
ErrVal LayerDecoder::xCreateData(const SequenceParameterSet& rcSPS)
{
    UInt uiIndex;
    ...
    {
        Frame::create(m_apcFrameTemp[ uiIndex ], *m_pcYuvFullPelBufferCtrl, *m_pcYuvFullPelBufferCtrl, FRAME, 0);
        m_apcFrameTemp[ uiIndex ]->init();
    }

    Frame::create(m_pcResidualLF,        *m_pcYuvFullPelBufferCtrl, *m_pcYuvFullPelBufferCtrl, FRAME, 0);
    Frame::create(m_pcResidualILPred,    *m_pcYuvFullPelBufferCtrl, *m_pcYuvFullPelBufferCtrl, FRAME, 0);
    Frame::create(m_pcILPrediction,      *m_pcYuvFullPelBufferCtrl, *m_pcYuvFullPelBufferCtrl, FRAME, 0);
    Frame::create(m_pcBaseLayerFrame,    *m_pcYuvFullPelBufferCtrl, *m_pcYuvFullPelBufferCtrl, FRAME, 0);
    Frame::create(m_pcBaseLayerResidual, *m_pcYuvFullPelBufferCtrl, *m_pcYuvFullPelBufferCtrl, FRAME, 0);
    m_pcResidualLF->init();
    m_pcResidualILPred->init();
    m_pcILPrediction->init();
...
ErrVal LayerDecoder::xDeleteData()
{
    UInt uiIndex;
    ...
    if(m_pcILPrediction)
    {
        m_pcILPrediction->uninit();
        m_pcILPrediction->destroy();
        m_pcILPrediction = 0;
    }
...
ErrVal LayerDecoder::xInitESSandCroppingWindow(SliceHeader&  rcSliceHeader,
...
ErrVal LayerDecoder::xInitBaseLayer(ControlData&  rcControlData,
                                    Bool  bFirstSliceInLayerRepresentation)
{
    //===== init =====
    ...
    if(pcBaseDataCtrl)
    {
        ...
        m_pcBaseLayerCtrl->upsampleMotion(pcSliceHeader, &m_cResizeParameters, pcBaseDataCtrl, pcList0, pcList1, m_cResizeParameters.m_bFieldPicFlag);
        ...
        if(m_cResizeParameters.m_bIsMbAffFrame && !bSNR)
        {
            ...
            m_pcBaseLayerCtrlField->upsampleMotion(pcSliceHeader, &m_cResizeParameters, pcBaseDataCtrl, pcList0, pcList1, true);
    ...
    if(pcBaseResidual)
    {
        ...
        {
          m_pcBaseLayerResidual->residualUpsampling(pcBaseResidual, m_cDownConvert, &m_cResizeParameters, pcBaseDataCtrl);
        }
    ...
    if(pcBaseFrame)
    {
        if(m_bFirstILPredSliceInLayerRep)
        {
            ...
            m_pcBaseLayerFrame->intraUpsampling(pcBaseFrame, pcTempBaseFrame, pcTempFrame, m_cDownConvert, &m_cResizeParameters,
...
Void LayerDecoder::xSetMCResizeParameters(ResizeParameters& rcResizeParameters)
{
    m_pcMotionCompensation->setResizeParameters(&rcResizeParameters);
}


ErrVal LayerDecoder::xGetBaseLayerData(ControlData&      rcControlData,
                                       Frame*&           rpcBaseFrame,
...

{
    ...
    m_pcH264AVCDecoder->getBaseLayerData(*pcSliceHeader,
                                             rpcBaseFrame,
                                             rpcBaseResidual,
                                             rpcBaseDataCtrl,
                                             rcResizeParameters,
