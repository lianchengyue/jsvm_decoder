                      ...
                      UInt& ruiNextSkippedVLC)
{
    ...
    if(bIsCoded)
    {
        ...
        {
            m_pcMbSymbolReadIf->fieldFlag(rcMbDataAccess);
        }
...
ErrVal MbParser::xReadReferenceIndices(MbDataAccess& rcMbDataAccess, MbMode eMbMode, ListIdx eLstIdx)
{
...
ErrVal MbParser::xReadTextureInfo(MbDataAccess&   rcMbDataAccess,
                                   Bool bTrafo8x8Flag,
...
Bool MbParser::xCheckSkipSliceMb(MbDataAccess& rcMbDataAccess, UInt uiNumMbRead, Bool& rbEndOfSlice)
{
...
ErrVal MbParser::xSkipMb(MbDataAccess& rcMbDataAccess)
{
    if(rcMbDataAccess.getSH().isBSlice())
    {
        rcMbDataAccess.setConvertMbType(0);
    }
    else
    {
        rcMbDataAccess.getMbMotionData(LIST_0).setRefIdx(1);
        rcMbDataAccess.setConvertMbType(MSYS_UINT_MAX);
    }
