...
ErrVal MbDecoder::xPredictionFromBaseLayer(MbDataAccess&  rcMbDataAccess,
            ...
            case MODE_8x8ref0:
              {
                for(B8x8Idx c8x8Idx; c8x8Idx.isLegal(); c8x8Idx++)
                {
                  if(rcMbData.getBlkMode(c8x8Idx.b8x8Index()) != BLK_SKIP
                       &&  rcMbData.isBlockFwdBwd(c8x8Idx.b8x8Index(), eListIdx)
                       &&  rcMbMotionData.getMotPredFlag(c8x8Idx.b8x8()))
                  {
                    ROT(bFieldMismatch);
                    ROF(pcMbDataAccessBase);
...
ErrVal MbDecoder::decode(MbDataAccess&  rcMbDataAccess,
                         MbDataAccess*  pcMbDataAccessBase,
                         Frame*         pcFrame,
                         Frame*         pcResidualLF,
                         ...
                         Bool           bReconstructAll)
{
    ...
    xPredictionFromBaseLayer(rcMbDataAccess, pcMbDataAccessBase);

    rcMbDataAccess.setMbDataAccessBase(pcMbDataAccessBase);

    YuvMbBuffer  cPredBuffer;
    ...
    if(rcMbDataAccess.getMbData().isIntra())
    {
        ...
        if(pcResidualLF || pcResidualILPred)
        {
            ...
            if(pcResidualLF)
            {
                pcResidualLF->getFullPelYuvBuffer()->loadBuffer(&cYuvMbBuffer);
            }
            if(pcResidualILPred)
            {
                pcResidualILPred->getFullPelYuvBuffer()->loadBuffer(&cYuvMbBuffer);
            }
        }
        ...
        else
        {
            ...
            YuvMbBuffer cRecBuffer;
            cRecBuffer.loadIntraPredictors(pcFrame->getFullPelYuvBuffer());

            if(rcMbDataAccess.getMbData().isIntra16x16())
            {
                //===== I_16x16 mode ====
                xDecodeMbIntra16x16(rcMbDataAccess, cRecBuffer, cPredBuffer);
            }
            else if(rcMbDataAccess.getMbData().isTransformSize8x8())
            {
                //===== I_8x8 mode =====
                xDecodeMbIntra8x8(rcMbDataAccess, cRecBuffer, cPredBuffer);
            }
            else
            {
                //===== I_4x4 mode =====
                xDecodeMbIntra4x4(rcMbDataAccess, cRecBuffer, cPredBuffer);
            }
            pcFrame->getFullPelYuvBuffer()->loadBuffer(&cRecBuffer);
            rcMbDataAccess.getMbTCoeffs().copyPredictionFrom(cRecBuffer);
        }
    ...
    else
    {
        ...
        xDecodeMbInter(rcMbDataAccess, pcMbDataAccessBase,
    ...
    rcMbDataAccess.getMbMotionData(LIST_0).setRefPicIdcs(rcMbDataAccess.getSH().getRefFrameList(rcMbDataAccess.getMbPicType(), LIST_0));
    rcMbDataAccess.getMbMotionData(LIST_1).setRefPicIdcs(rcMbDataAccess.getSH().getRefFrameList(rcMbDataAccess.getMbPicType(), LIST_1));

    return Err::m_nOK;
}


ErrVal MbDecoder::calcMv(MbDataAccess& rcMbDataAccess, MbDataAccess* pcMbDataAccessBaseMotion)
{
    ...
    else
    {
        if(rcMbDataAccess.getMbData().getMbMode() == MODE_8x8 || rcMbDataAccess.getMbData().getMbMode() == MODE_8x8ref0)
        {
            for(B8x8Idx c8x8Idx; c8x8Idx.isLegal(); c8x8Idx++)
            {
                ...
                m_pcMotionCompensation->calcMvSubMb(c8x8Idx, rcMbDataAccess, pcMbDataAccessBaseMotion);
            }
        }
        else
        {
            //----- motion compensated prediction -----
            m_pcMotionCompensation->calcMvMb(rcMbDataAccess, pcMbDataAccessBaseMotion);
        }
    }
...
ErrVal MbDecoder::xDecodeMbPCM(MbDataAccess& rcMbDataAccess, YuvMbBuffer& rcYuvMbBuffer)
{
                                 ...
                                 Frame*         pcBaseLayerRec)
{
    ...
    YuvMbBuffer  cYuvMbBufferResidual;
    cYuvMbBufferResidual.setAllSamplesToZero();
    ...
    if(bReconstruct)
    {
        if(rcMbDataAccess.getMbData().getMbMode() == MODE_8x8 || rcMbDataAccess.getMbData().getMbMode() == MODE_8x8ref0)
        {
          ...
          {
              //----- motion compensated prediction -----
              m_pcMotionCompensation->compensateSubMb(c8x8Idx,
        ...
        {
            //----- motion compensated prediction -----
            m_pcMotionCompensation->compensateMb(rcMbDataAccess,
                                                 rcRefFrameList0,
                                                 rcRefFrameList1,
                                                 &cYuvMbBuffer,
                                                 bCalcMv);
        ...
        if(pcBaseLayerRec)
        {
            m_pcMotionCompensation->compensateMbBLSkipIntra(rcMbDataAccess, &cYuvMbBuffer, pcBaseLayerRec);
        }
    ...
    if(rcMbDataAccess.getMbData().isTransformSize8x8())
    {
        ...
        {
            m_pcTransform->invTransform8x8Blk(cYuvMbBufferResidual.getYBlk(cIdx),
    ...
    else
    {
        for(B4x4Idx cIdx; cIdx.isLegal(); cIdx++)
        {
            m_pcTransform->invTransform4x4Blk(cYuvMbBufferResidual.getYBlk(cIdx),
                                              cYuvMbBufferResidual.getLStride(),
                                              rcCoeffs.get(cIdx));
        }
    }
    ...
    xDecodeChroma(rcMbDataAccess, cYuvMbBufferResidual, rcPredBuffer, uiChromaCbp, false);
    ...
    {
        m_pcMotionCompensation->updateMbBLSkipResidual(rcMbDataAccess, cResidual);
    }
    ...
    if(pcResidualILPred)
    {
        pcResidualILPred->getFullPelYuvBuffer()->loadBuffer(&cResidualILPred);
    }
                                    ...
                                    YuvMbBuffer&   rcPredBuffer)
{
    ...
    for(B4x4Idx cIdx; cIdx.isLegal(); cIdx++)
    {
        ...
        m_pcIntraPrediction->predictLumaBlock(puc, iStride, uiPredMode, cIdx);

        rcPredBuffer.loadLuma(cYuvMbBuffer, cIdx);

        if(rcMbDataAccess.getMbData().is4x4BlkCoded(cIdx))
        {
            m_pcTransform->invTransform4x4Blk(puc, iStride, rcCoeffs.get(cIdx));
        }
#endif
    }

    UInt uiChromaCbp = rcMbDataAccess.getMbData().getCbpChroma4x4();
    xDecodeChroma(rcMbDataAccess, cYuvMbBuffer, rcPredBuffer, uiChromaCbp, true);
                                    ...
                                    YuvMbBuffer&  rcPredBuffer)
{
  ...
  Int  iStride = cYuvMbBuffer.getLStride();
    ...
    for(B8x8Idx cIdx; cIdx.isLegal(); cIdx++)
    {
      ...
      m_pcIntraPrediction->predictLuma8x8Block(puc, iStride, uiPredMode, cIdx);

      rcPredBuffer.loadLuma(cYuvMbBuffer, cIdx);

      if(rcMbDataAccess.getMbData().is4x4BlkCoded(cIdx))
      {
          m_pcTransform->invTransform8x8Blk(puc, iStride, rcCoeffs.get8x8(cIdx));
      }
#endif
    }
                                   ...
                                   YuvPicBuffer*  pcBaseYuvBuffer)
{
    ...
    if(rcMbDataAccess.getMbData().isTransformSize8x8())
    {
        for(B8x8Idx cIdx; cIdx.isLegal(); cIdx++)
        {
            m_pcTransform->invTransform8x8Blk(cYuvMbBuffer.getYBlk(cIdx),
    ...
    else
    {
        for(B4x4Idx cIdx; cIdx.isLegal(); cIdx++)
        {
            m_pcTransform->invTransform4x4Blk(cYuvMbBuffer.getYBlk(cIdx),
                                              cYuvMbBuffer.getLStride(),
                                              rcCoeffs.get(cIdx));
        }
    }
    ...
    UInt uiChromaCbp = rcMbDataAccess.getMbData().getCbpChroma4x4();
    xDecodeChroma(rcMbDataAccess, cYuvMbBuffer, rcPredBuffer, uiChromaCbp, false, bAddBaseCoeffs);
                                      ...
                                      YuvMbBuffer&  rcPredBuffer)
{
    ...
    m_pcTransform->invTransformDcCoeff(rcCoeffs.get(B4x4Idx(0)), iQpScale, iQp/6);

    for(B4x4Idx cIdx; cIdx.isLegal(); cIdx++)
    {
        m_pcTransform->invTransform4x4Blk(cYuvMbBuffer.getYBlk(cIdx), iStride, rcCoeffs.get(cIdx));
    }
                                ...
                                Bool          bAddBaseCoeffsChroma)
{
    ...
    if(bPredChroma)
    {
        m_pcIntraPrediction->predictChromaBlock(pucCb, pucCr, iStride, rcMbDataAccess.getMbData().getChromaPredMode());
    ...
    m_pcTransform->invTransformChromaBlocks(pucCb, iStride, rcCoeffs.get(CIdx(0)));
    m_pcTransform->invTransformChromaBlocks(pucCr, iStride, rcCoeffs.get(CIdx(4)));
...
ErrVal MbDecoder::xScale4x4Block(TCoeff*  piCoeff,
...
ErrVal MbDecoder::xScale8x8Block(TCoeff*  piCoeff,
                                 const UChar*  pucScale,
                                 const QpParameter& rcQP)
{
...
ErrVal MbDecoder::xScaleTCoeffs(MbDataAccess& rcMbDataAccess)
{
    ...
    for(CIdx cIdx; cIdx.isLegal(); cIdx++)
    {
        ...
        xScale4x4Block(rcTCoeffs.get(cIdx), pucScale, 1, rcQp);
...
ErrVal MbDecoder::xAddTCoeffs(MbDataAccess& rcMbDataAccess, MbDataAccess& rcMbDataAccessBase)
{
    if(rcMbDataAccess.getMbData().isPCM())
    {
