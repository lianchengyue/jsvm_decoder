...
Void NonVCLNALUnit::destroyNALOnly()
{
    m_pcBinData       = 0;
    delete this;
}
...
SliceDataNALUnit::SliceDataNALUnit(BinData* pcBinDataPrefix,
                                   const PrefixHeader& rcPrefixHeader,
                                   BinData* pcBinData,
...
ErrVal AccessUnit::update(BinData* pcBinData, PrefixHeader& rcPrefixHeader)
{
    ROT(m_bComplete);
    ROT(m_bEndOfStream);
    ROF(pcBinData);
...
ErrVal AccessUnit::update(BinData* pcBinData, SliceHeader& rcSliceHeader)
{
    ROT(m_bComplete);
    ROT(m_bEndOfStream);
    ROF(pcBinData);
...
ErrVal AccessUnit::update(BinData* pcBinData, Bool bSEI, Bool bScalableSEI, Bool bBufferingPeriod)
{
    ROT(m_bComplete);
    ROT(m_bEndOfStream);
...
ErrVal AccessUnit::getAndRemoveNextNalUnit(NALUnit*& rpcNalUnit)
{
    ROF(m_bComplete);
...
Void AccessUnit::xSetComplete(SliceDataNALUnit* pcFirstSliceDataNALUnitOfNextAccessUnit,
...
MyList<NALUnit*>::iterator getNextVCL(MyList<NALUnit*>::iterator iIter, MyList<NALUnit*>::iterator iEnd, Bool bFirst = false)
{
    if(iIter != iEnd)
    {
      if(! bFirst)
...
Void AccessUnit::xRemoveRedundant()
{
...
Void AccessUnit::xSetParameters()
{
...
ErrVal CreaterH264AVCDecoder::create(CreaterH264AVCDecoder*& rpcCreaterH264AVCDecoder)
{
    ...
    rpcCreaterH264AVCDecoder->xCreateDecoder();
    return Err::m_nOK;
}

ErrVal CreaterH264AVCDecoder::xCreateDecoder()
{
    ParameterSetMng::create(m_pcParameterSetMngAUInit);
    ParameterSetMng::create(m_pcParameterSetMngDecode);
    BitReadBuffer::create(m_pcBitReadBuffer);
    NalUnitParser::create(m_pcNalUnitParser);
    SliceReader::create(m_pcSliceReader);
    SliceDecoder::create(m_pcSliceDecoder);
    UvlcReader::create(m_pcUvlcReader);
    CabacReader::create(m_pcCabacReader);
    MbParser::create(m_pcMbParser);
    MbDecoder::create(m_pcMbDecoder);
    LoopFilter::create(m_pcLoopFilter);
    IntraPrediction::create(m_pcIntraPrediction);
    MotionCompensation::create(m_pcMotionCompensation);
    H264AVCDecoder::create(m_pcH264AVCDecoder);
    ControlMngH264AVCDecoder::create(m_pcControlMng);
...
#ifdef SHARP_AVC_REWRITE_OUTPUT
    RewriteEncoder::create(m_pcRewriteEncoder);
#endif

    for(UInt uiLayer = 0; uiLayer < MAX_LAYERS; uiLayer++)
    {
        DecodedPicBuffer::create(m_apcDecodedPicBuffer[uiLayer]);
        LayerDecoder::create(m_apcLayerDecoder[uiLayer]);
...
{
    m_pcSliceDecoder->destroy();
    m_pcSliceReader->destroy();
    m_pcBitReadBuffer->destroy();
    m_pcUvlcReader->destroy();
    m_pcMbParser->destroy();
    m_pcLoopFilter->destroy();
    m_pcMbDecoder->destroy();
    m_pcTransform->destroy();
    m_pcIntraPrediction->destroy();
    m_pcMotionCompensation->destroy();
    m_pcQuarterPelFilter->destroy();
    m_pcCabacReader->destroy();
    m_pcNalUnitParser->destroy();
    m_pcParameterSetMngAUInit->destroy();
    m_pcParameterSetMngDecode->destroy();
    m_pcSampleWeighting->destroy();
    m_pcH264AVCDecoder->destroy();
    m_pcControlMng->destroy();
    m_pcReconstructionBypass->destroy();
#ifdef SHARP_AVC_REWRITE_OUTPUT
    m_pcRewriteEncoder->destroy();
#endif

    for(UInt uiLayer = 0; uiLayer < MAX_LAYERS; uiLayer++)
    {
        m_apcDecodedPicBuffer[uiLayer]->destroy();
        m_apcLayerDecoder[uiLayer]->destroy();
...
ErrVal CreaterH264AVCDecoder::init(Bool bOpenTrace)
{
    ...
    m_pcBitReadBuffer         ->init();
    m_pcNalUnitParser         ->init(m_pcBitReadBuffer, m_pcUvlcReader);
    m_pcUvlcReader            ->init(m_pcBitReadBuffer);
    m_pcCabacReader           ->init(m_pcBitReadBuffer);
    m_pcQuarterPelFilter      ->init();
    m_pcParameterSetMngAUInit ->init();
    m_pcParameterSetMngDecode ->init();
    m_pcSampleWeighting       ->init();
    m_pcSliceDecoder          ->init(m_pcMbDecoder, m_pcControlMng);
    m_pcSliceReader           ->init(m_pcMbParser);
    m_pcMbParser              ->init();
    m_pcLoopFilter            ->init(m_pcControlMng, m_pcReconstructionBypass, false);
    m_pcIntraPrediction       ->init();
    m_pcMotionCompensation    ->init(m_pcQuarterPelFilter, m_pcTransform, m_pcSampleWeighting);
    m_pcMbDecoder             ->init(m_pcTransform, m_pcIntraPrediction, m_pcMotionCompensation);
    m_pcH264AVCDecoder        ->init(m_pcNalUnitParser,
                                            m_pcUvlcReader,
...
#ifdef SHARP_AVC_REWRITE_OUTPUT
    m_pcRewriteEncoder->init();
#endif

    for(UInt uiLayer = 0; uiLayer < MAX_LAYERS; uiLayer++)
    {
        m_apcDecodedPicBuffer[uiLayer]->init (m_apcYuvFullPelBufferCtrl [uiLayer], uiLayer);
#ifdef SHARP_AVC_REWRITE_OUTPUT
        m_apcLayerDecoder[uiLayer]->init (uiLayer,
                                          m_pcH264AVCDecoder,
                                          m_pcNalUnitParser,
                                          m_pcSliceReader,
                                          m_pcSliceDecoder,
    ...
    m_pcControlMng->init(m_pcUvlcReader,
                         m_pcMbParser,
                         m_pcMotionCompensation,
...
ErrVal CreaterH264AVCDecoder::uninit(Bool bCloseTrace)
{
    ...
    m_pcParameterSetMngAUInit ->uninit();
    m_pcParameterSetMngDecode ->uninit();
    m_pcSliceDecoder          ->uninit();
    m_pcSliceReader           ->uninit();
    m_pcNalUnitParser         ->uninit();
    m_pcBitReadBuffer         ->uninit();
    m_pcUvlcReader            ->uninit();
    m_pcMbParser              ->uninit();
    m_pcLoopFilter            ->uninit();
    m_pcMbDecoder             ->uninit();
    m_pcIntraPrediction       ->uninit();
    m_pcMotionCompensation    ->uninit();
    m_pcCabacReader           ->uninit();
    m_pcH264AVCDecoder        ->uninit();
    m_pcControlMng            ->uninit();
    m_pcReconstructionBypass  ->uninit();
#ifdef SHARP_AVC_REWRITE_OUTPUT
    m_pcRewriteEncoder        ->uninit();
#endif

    for(UInt uiLayer = 0; uiLayer < MAX_LAYERS; uiLayer++)
    {
        m_apcDecodedPicBuffer    [uiLayer] ->uninit();
        m_apcLayerDecoder        [uiLayer] ->uninit();
...
ErrVal CreaterH264AVCDecoder::initNALUnit(BinData*& rpcBinData, AccessUnit& rcAccessUnit)
{
    m_pcH264AVCDecoder->initNALUnit(rpcBinData, rcAccessUnit);

    return Err::m_nOK;
}

ErrVal CreaterH264AVCDecoder::processNALUnit(PicBuffer*         pcPicBuffer,
                                             PicBufferList&     rcPicBufferOutputList,
                                             PicBufferList&     rcPicBufferUnusedList,
                                             BinDataList&       rcBinDataList,
                                             NALUnit&           rcNALUnit)
{
    m_pcH264AVCDecoder->processNALUnit(pcPicBuffer, rcPicBufferOutputList, rcPicBufferUnusedList, rcBinDataList, rcNALUnit);
    return Err::m_nOK;
}
...
ErrVal H264AVCPacketAnalyzer::create(H264AVCPacketAnalyzer*& rpcH264AVCPacketAnalyzer)
{
    rpcH264AVCPacketAnalyzer = new H264AVCPacketAnalyzer;
    ROT (NULL == rpcH264AVCPacketAnalyzer);
    rpcH264AVCPacketAnalyzer->xCreate();
    return Err::m_nOK;
}



ErrVal H264AVCPacketAnalyzer::xCreate()
{
    BitReadBuffer::create(m_pcBitReadBuffer);
    UvlcReader::create(m_pcUvlcReader);
    NalUnitParser::create(m_pcNalUnitParser);
...
ErrVal H264AVCPacketAnalyzer::destroy()
{
    m_pcBitReadBuffer->destroy();
    m_pcUvlcReader->destroy();
    m_pcNalUnitParser->destroy();
...
ErrVal H264AVCPacketAnalyzer::init()
{
  m_pcBitReadBuffer->init();
  m_pcUvlcReader->init(m_pcBitReadBuffer);
  m_pcNalUnitParser->init(m_pcBitReadBuffer, m_pcUvlcReader);
...
ErrVal H264AVCPacketAnalyzer::uninit()
{
  m_pcBitReadBuffer->uninit();
  m_pcUvlcReader->uninit();
  m_pcNalUnitParser->uninit();
...
ErrVal H264AVCPacketAnalyzer::process(BinData*            pcBinData,
                                      ...
                                      SEI::SEIMessage*&   pcScalableSEIMessage)
{
    ...
    if(eNalUnitType == NAL_UNIT_SEI)
    {
        ...
        SEI::read(m_pcUvlcReader, cMessageList, pcParameterSetMng);
    ...
    else if(eNalUnitType == NAL_UNIT_SPS || eNalUnitType == NAL_UNIT_SUBSET_SPS)
    {
        ...
        pcSPS->read(m_pcUvlcReader, eNalUnitType, bCompletelyParsed);
    ...
    else if(eNalUnitType == NAL_UNIT_PPS)
    {
        ...
        pcPPS->read(m_pcUvlcReader, eNalUnitType);
            ...
            eNalUnitType == NAL_UNIT_CODED_SLICE_SCALABLE   )
    {
        ...
        {
            UInt uiTemp;
            m_pcUvlcReader->getUvlc(uiTemp,  "SH: first_mb_in_slice");
            rcPacketDescription.uiFirstMb = uiTemp;

            m_pcUvlcReader->getUvlc(uiTemp,  "SH: slice_type");
            m_pcUvlcReader->getUvlc(uiPPSid, "SH: pic_parameter_set_id");
